...
describe('E2E тест навігації з мок даними', () => {
  ...
  beforeEach(() => {
    // Мок для списку — перехоплюємо і замінюємо відповідь, але дозволяємо запит
    cy.intercept('POST', '/api/animals/filter', req => {
      req.reply(res => {
        // Замінюємо body відповіді на мок-дані
        res.body = {
          animals: animalsMock,
          totalCount: animalsMock.length,
        };
      });
    }).as('getAnimalsList');

    cy.intercept('GET', '/api/animals/**', {
      statusCode: 200,
      body: animalsMock[0],
    }).as('getAnimalDetails');
    // Заглушка для /api/auth/refresh
  ...
  it('Відображає список тварин при переході на /animals', () => {
    cy.visit('/animals');
    cy.wait('@getAnimalsList');

    cy.get('app-animal-card').should('exist');
