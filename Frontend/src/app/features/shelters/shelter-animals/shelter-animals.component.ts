...
@Component({
  ...
  styleUrl: './shelter-animals.component.css',
...
export class ShelterAnimalsComponent {
  ...
  private updateUrl() {
    ...
    if (costs.length > 0) params.set('cost', costs.join(','));
    ...
    if (!this.sterelisationOptions()) params.set('sterilized', 'false');
    if (!this.availableForCareOptions()) params.set('undercare', 'true');
  ...
  backBottomClick() {
    const slugValue = this.slug();
    if (slugValue) this.router.navigate([`/shelters/${slugValue}`]);
  }
}
